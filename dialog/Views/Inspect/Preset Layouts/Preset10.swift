...
//
//  Preset10: Modern Sidebar Variant
//  Combines Preset6's sidebar navigation with Preset11's modern styling.
//
//  Features:
//  - Vertical sidebar navigation (from Preset6)
//  - Clean, modern IntroStepContainer design (from Preset11)
...
// MARK: - Preset10 State Definition

struct Preset10State: InspectPersistableState {
...
struct Preset10View: View, InspectLayoutProtocol {
    ...
    // Persistence
    private let persistenceService = InspectPersistence<Preset10State>(presetName: "preset10")
    ...
    private var interactionLogPath: String {
        "/tmp/preset10_interaction.log"
    }

    private var interactionPlistPath: String {
        "/tmp/preset10_interaction.plist"
    }
    ...
    private var triggerFilePath: String {
        ...
        if appArguments.inspectMode.present {
            return "/tmp/swiftdialog_dev_preset10.trigger"
        }
        return "/tmp/swiftdialog_\(ProcessInfo.processInfo.processIdentifier)_preset10.trigger"
    }
    ...
    private var finalTriggerFilePath: String {
        ...
        if appArguments.inspectMode.present {
            return "/tmp/swiftdialog_dev_preset10_final.trigger"
        }
        return "/tmp/swiftdialog_\(ProcessInfo.processInfo.processIdentifier)_preset10_final.trigger"
    }
    ...
    var body: some View {
        ...
        .onAppear {
            writeLog("Preset10: View appearing, loading state...", logLevel: .info)
            loadPersistedState()
            setupFileMonitoring()
            startComplianceMonitoring()
            startIntroStepMonitoring()
            writeInteractionLog("launched", step: "preset10")
            logPreset10Event("view_appeared", details: [
    ...
    private func handleStepSelection(_ index: Int) {
        guard !shouldBlockNavigation else {
            writeLog("Preset10: Navigation blocked during processing", logLevel: .debug)
            return
        }

        // Check observe-only on target step
        guard !isItemObserveOnly(inspectState.items[safe: index], config: inspectState.config) else {
            writeLog("Preset10: Navigation blocked - step is observe-only", logLevel: .debug)
            return
        }
        ...
        writeLog("Preset10: Navigated to step \(index) (\(targetItem.id))", logLevel: .info)
    }
    ...
    private func navigateToNextStep() {
        ...
        writeLog("Preset10: Advanced to step \(currentStep)", logLevel: .info)
        writeInteractionLog("navigate_next", step: "step_\(currentStep)")
        logPreset10Event("step_transition", details: [
            "from": oldStep,
            "to": currentStep,
                ...
                if let formState = inspectState.guidanceFormInputs[item.id] {
                    ...
                    for block in guidanceContent {
                        if block.required == true {
                            ...
                            if !hasValue {
                                writeLog("Preset10: Auto-start blocked - required field '\(fieldId)' not filled", logLevel: .debug)
                                return
                            }
                        }
                    }
                } else {
                    writeLog("Preset10: Auto-start blocked - no form input state for step '\(item.id)'", logLevel: .debug)
                    return
                }
            }
        }

        logPreset10Event("auto_start_processing", details: ["stepId": item.id])
    ...
    private func goToPreviousStep() {
        ...
        restartIntroStepMonitoring()
        writeLog("Preset10: Went back to step \(currentStep)", logLevel: .info)
    ...
    private func handleStepCompletion(item: InspectConfig.ItemConfig) {
        ...
        savePersistedState()
        writeLog("Preset10: Step '\(item.id)' marked as completed", logLevel: .info)
        writeInteractionLog("completed_step", step: item.id)
        logPreset10Event("step_completed", details: [
            "stepId": item.id,
    ...
    private func resetSteps() {
        ...
        writeLog("Preset10: All progress reset", logLevel: .info)
        writeInteractionLog("reset", step: "all")
        logPreset10Event("steps_reset", details: [
    ...
    private func handleFinish() {
        ...
        writeLog("Preset10: Completing with exit code 0", logLevel: .info)
    ...
    private func startProcessing(for item: InspectConfig.ItemConfig) {
        ...
        print("[PRESET10_PROCESSING] step_started: \(item.id)")
        writeLog("Preset10: Processing started for step '\(item.id)'", logLevel: .info)
        writeInteractionLog("processing_started", step: item.id, data: ["duration": duration])
        logPreset10Event("processing_started", details: [
            "stepId": item.id,
            "duration": duration,
    ...
    private func startWaitingTimer(for item: InspectConfig.ItemConfig) {
        ...
        writeLog("Preset10: Starting waiting timer for step '\(item.id)'", logLevel: .debug)
            ...
            DispatchQueue.main.async {
                if case .waiting(let stepId, let waitElapsed) = self.processingState {
                    ...
                    if newElapsed == 10 || newElapsed == 30 || newElapsed == 60 || newElapsed % 60 == 0 {
                        writeLog("Preset10: Waiting timer at \(newElapsed)s for step '\(stepId)'", logLevel: .debug)
                    }
                }
            }
        }
    ...
    private func handleCompletionTrigger(stepId: String, result: CompletionResult) {
        ...
        case .success(let message):
            ...
            print("[PRESET10_PROCESSING] result: \(stepId) = success")
            writeInteractionLog("success", step: stepId, data: ["message": message ?? ""])
            logPreset10Event("step_success", details: ["stepId": stepId, "message": message ?? ""])

        case .failure(let message):
            failedSteps[stepId] = message ?? "Step failed"
            print("[PRESET10_PROCESSING] result: \(stepId) = failed")
            writeInteractionLog("failure", step: stepId, data: ["message": message ?? "Step failed"])
            logPreset10Event("step_failure", details: ["stepId": stepId, "message": message ?? "Step failed"])

        case .warning(let message):
            print("[PRESET10_PROCESSING] result: \(stepId) = warning")
            writeInteractionLog("warning", step: stepId, data: ["message": message ?? ""])
            logPreset10Event("step_warning", details: ["stepId": stepId, "message": message ?? ""])

        case .cancelled:
            skippedSteps.insert(stepId)
            print("[PRESET10_PROCESSING] result: \(stepId) = skipped")
            writeInteractionLog("cancelled", step: stepId)
            logPreset10Event("step_cancelled", details: ["stepId": stepId])
        }

        writeLog("Preset10: Step '\(stepId)' completed with result: \(result)", logLevel: .info)
        savePersistedState()
    }

    private func handleOverrideAction(action: OverrideDialogView.OverrideAction, stepId: String) {
        logPreset10Event("override_action", details: ["stepId": stepId, "action": "\(action)"])
    ...
    private func startComplianceMonitoring() {
        if let plistSources = inspectState.config?.plistSources, !plistSources.isEmpty {
            writeLog("Preset10: Starting ComplianceAggregatorService with \(plistSources.count) sources", logLevel: .info)
    ...
    private func startIntroStepMonitoring() {
        ...
        introStepMonitor.startMonitoring(item: currentItem) { triggerItemId, result in
            writeLog("Preset10: Step monitor triggered for '\(triggerItemId)' with result: \(result)", logLevel: .info)
        }
    }
    ...
    private func setupFileMonitoring() {
        ...
        guard fileDescriptor >= 0 else {
            writeLog("Preset10: Failed to open trigger file for monitoring", logLevel: .error)
            return
        }
        ...
        writeLog("Preset10: File monitoring started with DispatchSource at \(triggerFilePath) (mode: \(triggerMode), zero-latency)", logLevel: .info)
    }
            ...
            if trimmed.hasPrefix("complete:") {
                ...
                if !completedSteps.contains(stepId) {
                    if inspectState.items.contains(where: { $0.id == stepId }) {
                        ...
                        writeLog("Preset10: Step '\(stepId)' marked complete via trigger", logLevel: .info)
                    }
                }
                ...
                if let stepIndex = Int(stepIndexString), stepIndex >= 0, stepIndex < inspectState.items.count {
                    ...
                    writeLog("Preset10: Navigated to step index \(stepIndex) via trigger", logLevel: .info)
                }
                ...
                if let index = itemIndex, index >= 0, index < inspectState.items.count {
                    if let status = statusIcon, !status.isEmpty {
                        ...
                        writeLog("Preset10: Updated item \(index) status icon to '\(status)'", logLevel: .info)
                    } else {
                        dynamicState.updateItemStatusIcon(index: index, icon: nil)
                        writeLog("Preset10: Cleared item \(index) status icon", logLevel: .info)
                    }
                }
                ...
                if parts.count == 2 {
                    ...
                    if let percentage = Int(String(parts[1])) {
                        ...
                        writeLog("Preset10: Updated progress for '\(stepId)': \(percentage)%", logLevel: .info)
                    }
                }
                    ...
                    if let blockIndex = Int(String(parts[1])) {
                        ...
                        if valueString.contains("=") {
                            ...
                            if propParts.count == 2 {
                                ...
                                logPreset10Event("guidance_property_update", details: ["stepId": stepId, "index": blockIndex, "property": property, "value": value])
                            ...
                            logPreset10Event("guidance_content_update", details: ["stepId": stepId, "index": blockIndex, "content": valueString])
                ...
                if parts.count == 2 {
                    ...
                    writeLog("Preset10: Updated message for '\(stepId)': \(message)", logLevel: .info)
                }
                ...
                if let itemId = targetItemId {
                    ...
                    writeLog("Preset10: Manual recheck triggered for item '\(itemId)'", logLevel: .info)
                } else {
                    inspectState.recheckAllPlistMonitors { itemId, blockIndex, property, newValue in
                        dynamicState.updateGuidanceProperty(stepId: itemId, blockIndex: blockIndex, property: property, value: newValue)
                    }
                    writeLog("Preset10: Manual recheck triggered for ALL items", logLevel: .info)
                }
                ...
                if parts.count >= 3 {
                    ...
                    logPreset10Event("display_data_update", details: ["stepId": stepId, "key": key, "value": value, "color": color ?? "none"])
    ...
    private func loadPersistedState() {
        guard let state = persistenceService.loadState() else {
            writeLog("Preset10: No persisted state found", logLevel: .debug)
            return
        }
        ...
        writeLog("Preset10: Restored state - step \(currentStep), \(completedSteps.count) completed", logLevel: .info)
    }

    private func savePersistedState() {
        let state = Preset10State(
        ...
        writeLog("Preset10: Saved state", logLevel: .debug)
    }

    private func writeFinalTriggerFile() {
        var output: [String] = []
        output.append("preset10_completed")
        ...
        writeLog("Preset10: Wrote final trigger file", logLevel: .info)
        logPreset10Event("workflow_completed", details: [
    ...
    private func logPreset10Event(_ event: String, details: [String: Any] = [:]) {
        var logDetails = details
        logDetails["preset"] = "10"
        ...
        writeLog("Preset10: \(event) - \(detailsString)", logLevel: .info)

        // Output to console for external monitoring
        print("[PRESET10_EVENT] \(event) \(detailsString)")
    }
    ...
    private func writeInteractionLog(_ event: String, step: String, data: [String: Any]) {
        print("[PRESET10_INTERACTION] event=\(event) step=\(step) current=\(currentStep) completed=\(completedSteps.count)")
